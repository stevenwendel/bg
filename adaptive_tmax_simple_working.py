#!/usr/bin/env python3
"""
Simple Working Genetic Algorithm with Adaptive TMAX and Multiprocessing

This version focuses on reliable speedup without complex optimizations that cause Numba issues.
- ✅ Adaptive TMAX with fixed epochs (1.67x speedup)
- ✅ Multiprocessing (3-10x speedup depending on cores)
- ✅ All DNA vectors and scores saved
- ❌ Complex optimizations that cause Numba compatibility issues

Expected combined speedup: 5-15x over baseline
"""
...
# ============================================================================
# Core Simulation Functions (from working implementation)
# ============================================================================

@njit(parallel=False, fastmath=True, cache=True)
def step_kernel(V, U, Ibuf, t_ptr, a, b, vreset, d, k, vr, vt, vpeak, C, E, W, alpha):
    """Simple step kernel without complex optimizations."""
    ...
    for k_shift in range(L):
        ...
        for i in range(n):
            Ibuf[target_idx, i] += post_I[i] * alpha[k_shift]
...
@njit(fastmath=True, cache=True)
def simulate_simple(W, a, b, vreset, d, k, vr, vt, vpeak, C, E, alpha, cue_wave, go_wave, 
                   crit_Exp, crit_Cont, crit_indices, pass_ids, tmax, control):
    """Simple simulation without complex optimizations."""
    ...
    for t in range(tmax):
        ...
        if cidx == (BIN_SIZE - 1):
            ...
            bin_score = score_bin(curr_bin_results, crits, crit_indices, bin, pass_ids)
            score += bin_score
            bin += 1
...
def create_matrices(dna_vectors: np.array, conn_map: np.array, num_neurons: int):
    """Create weight matrices from DNA vectors."""
    N = num_neurons
...
@njit(parallel=False, fastmath=True, cache=True)
def evaluate_population_simple(population_vectors, conn_map, N, a, b, vreset, d, k, vr, vt, vpeak, C, E, 
                              alpha, cue_wave, go_wave, crit_Exp, crit_Cont, crit_indices, pass_ids, tmax):
    """Evaluate population without complex optimizations."""
    n_individuals = len(population_vectors)
    vectors_scores = np.zeros((n_individuals, 3), dtype=np.int32)
    
    # Create matrices for all individuals
    matrices = create_matrices(population_vectors, conn_map, N)
    
    for i in range(n_individuals):
        # Simulate experimental condition
        exp_score, _ = simulate_simple(
            matrices[i], a, b, vreset, d, k, vr, vt, vpeak, C, E,
            alpha, cue_wave, go_wave, crit_Exp, crit_Cont, crit_indices, pass_ids,
            tmax, False
        )
        
        # Simulate control condition
        cont_score, _ = simulate_simple(
            matrices[i], a, b, vreset, d, k, vr, vt, vpeak, C, E,
            alpha, cue_wave, go_wave, crit_Exp, crit_Cont, crit_indices, pass_ids,
            tmax, True
        )
            
        vectors_scores[i, 0] = i
        vectors_scores[i, 1] = exp_score
        vectors_scores[i, 2] = cont_score
...
# ============================================================================
# Worker Process Function (Simple Working Version)
# ============================================================================

def worker_process_simple(process_id: int, config_name: str, num_generations: int, seed_offset: int, 
                         results_dir: Path, shared_counter, lock, tmax_strategy: str = "progressive") -> Dict:
    """Simple worker function focused on reliability and speedup."""
    np.random.seed(int(time.time() * 1000) % (2**32) + seed_offset + process_id)
    random.seed(np.random.randint(0, 2**32))
    
    print(f"Process {process_id}: Starting SIMPLE WORKING GA with config '{config_name}', strategy '{tmax_strategy}'")
    ...
    try:
        ...
        # Evolution loop with adaptive TMAX
        for gen in range(num_generations):
            gen_start = time.time()
            
            # Adaptive TMAX with fixed epochs
            ...
            # Simple population evaluation
            scores = evaluate_population_simple(
            ...
            print(f"Process {process_id}: Gen {gen+1}/{num_generations} completed. "
                  f"TMAX={tmax}ms ({speedup:.1f}x speedup), "
...
def run_simple_working_ga(config_name: str = "medium", num_processes: Optional[int] = None, 
                         num_generations: Optional[int] = None, results_dir: Optional[str] = None,
                         tmax_strategy: str = "progressive") -> Dict:
    """Run simple working genetic algorithm with reliable multiprocessing and adaptive TMAX."""
    ...
    if results_dir is None:
        ...
        results_path = Path(f"results/simple_working_ga_{config_name}_{tmax_strategy}_{timestamp}")
    else:
        results_path = Path(results_dir)
    
    results_path.mkdir(parents=True, exist_ok=True)
    
    print("🚀⚡ SIMPLE WORKING GENETIC ALGORITHM ⚡🚀")
    print("=" * 80)
    ...
    print()
    print("Features:")
    print("  ✅ Adaptive simulation time (fixed epochs) - 1.67x speedup")
    print(f"  ✅ Multiprocessing ({num_processes} cores) - ~{num_processes}x speedup")
    print("  ✅ All DNA vectors and scores saved")
    print("  ✅ Reliable Numba JIT compilation")
    print()
    
    total_individuals = num_processes * cfg['POP_SIZE'] * num_generations
    print(f"Total individuals to test: {total_individuals:,}")
    print(f"Results directory: {results_path}")
    print(f"Expected overall speedup: {num_processes * 1.67:.1f}x")
    print("=" * 80)
    ...
    print(f"\\n🚀 Starting {num_processes} reliable processes...")
    with mp.Pool(processes=num_processes) as pool:
        results = pool.starmap(worker_process_simple, worker_args)
    ...
    print("\\n" + "=" * 80)
    print("🎉⚡ SIMPLE WORKING GA COMPLETED! ⚡🎉")
    print("=" * 80)
    ...
    print(f"Overall speedup (multiprocessing + adaptive): {(total_individuals / total_time) / 7.7:.1f}x")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run simple working genetic algorithm")
    ...
    parser.add_argument("--strategy", choices=["progressive", "exponential"], 
                       default="progressive", help="TMAX adaptation strategy")
    
    args = parser.parse_args()
    
    # Run the simple working GA
    results = run_simple_working_ga(
